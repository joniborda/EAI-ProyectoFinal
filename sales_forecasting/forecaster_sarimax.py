...
def _model_path(product_id: str) -> Path:
    ...
    return _settings.model_dir / f"sarimax_product_{sid}.joblib"


def train_and_save(y: np.ndarray, product_id: str, seasonal_periods: int = 7) -> Path:
    if y.size == 0:
...
def forecast(product_id: str, horizon_days: int) -> np.ndarray:
    fitted = load_model(product_id)
